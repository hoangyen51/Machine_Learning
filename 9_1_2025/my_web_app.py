...
def main():
    if "min_value" in request.form and "max_value" in request.form and "sort_order" in request.form:
        min_value = float(request.form["min_value"])
        max_value = float(request.form["max_value"])
        sort_order = request.form["sort_order"] == "asc"  # True if ascending, False if descending

        # Đọc dữ liệu từ CSV
        df = pd.read_csv('dataset/SalesTransactions.csv')

        # Gọi hàm xử lý
        result = find_and_sort_orders(df, min_value, max_value, sort_order)
